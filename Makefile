...
am__base_list = \
  sed '$$!N;$$!N;$$!N;$$!N;$$!N;$$!N;$$!N;s/\n/ /g' | \
  sed '$$!N;$$!N;$$!N;$$!N;s/\n/ /g'
am__installdirs = "$(DESTDIR)$(polarhrmdocdir)"
DATA = $(polarhrmdoc_DATA)
...
AMTAR = ${SHELL} /home/thomas/projects/polarhrm/missing --run tar
...
PACKAGE_STRING = polarhrm 2012-01-02
PACKAGE_TARNAME = polarhrm
PACKAGE_URL = 
PACKAGE_VERSION = 2012-01-02
PATH_SEPARATOR = :
...
install-polarhrmdocDATA: $(polarhrmdoc_DATA)
	@$(NORMAL_INSTALL)
	test -z "$(polarhrmdocdir)" || $(MKDIR_P) "$(DESTDIR)$(polarhrmdocdir)"
	@list='$(polarhrmdoc_DATA)'; test -n "$(polarhrmdocdir)" || list=; \
	for p in $$list; do \
	  if test -f "$$p"; then d=; else d="$(srcdir)/"; fi; \
	  echo "$$d$$p"; \
	done | $(am__base_list) | \
	while read files; do \
	  echo " $(INSTALL_DATA) $$files '$(DESTDIR)$(polarhrmdocdir)'"; \
	  $(INSTALL_DATA) $$files "$(DESTDIR)$(polarhrmdocdir)" || exit $$?; \
	done

uninstall-polarhrmdocDATA:
	@$(NORMAL_UNINSTALL)
	@list='$(polarhrmdoc_DATA)'; test -n "$(polarhrmdocdir)" || list=; \
	files=`for p in $$list; do echo $$p; done | sed -e 's|^.*/||'`; \
	test -n "$$files" || exit 0; \
	echo " ( cd '$(DESTDIR)$(polarhrmdocdir)' && rm -f" $$files ")"; \
	cd "$(DESTDIR)$(polarhrmdocdir)" && rm -f $$files
...
installdirs-am:
	for dir in "$(DESTDIR)$(polarhrmdocdir)"; do \
...
install-data-am: install-polarhrmdocDATA
...
uninstall-am: uninstall-local uninstall-polarhrmdocDATA
...
.PHONY: $(RECURSIVE_CLEAN_TARGETS) $(RECURSIVE_TARGETS) CTAGS GTAGS \
	...
	install-pdf install-pdf-am install-polarhrmdocDATA install-ps \
